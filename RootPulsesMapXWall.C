...
// .x RootPulsesMap.C+("118","france","tunnel")
...
void RootPulsesMapXWall (string run, string side, string wall)
{
  gROOT->Reset ();
  gStyle->SetOptStat (0);
  ...
  corrected_times.open("corrected_times_"+side+"_"+wall+".txt");
  ...
  if (wall=="mountain") w=0;
  else if (wall=="tunnel") w=1;
  else cout << "Wrong wall value!" << endl;

  TCanvas *canvas = new TCanvas ("canvas", "canvas", 200, 200, 1100, 800);

  // Root histos:
  TH2D *h2_counts = new TH2D ("counts","counts; Y; Z",  slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_amplitude = new TH2D ("h2_amplitude_first_peak","Amplitude_First_Peak; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_charge_first_peak = new TH2D ("h2_charge_first_peak","Charge_First_Peak; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_charge_second_peak = new TH2D ("h2_charge_second_peak","Charge_Second_Peak; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_amplitude_second_peak = new TH2D ("h2_amplitude_second_peak","Amplitude_Second_Peak; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_attenuation_amplitude = new TH2D ("attenuation_amplitude","Attenuation_Amplitude; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_attenuation_charge = new TH2D ("attenuation_amplitude","Attenuation_Amplitude; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_time_difference_CFD = new TH2D ("time_diff_CFD","time_diff_CFD; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_time_difference_max = new TH2D ("time_diff_max","time_diff_max; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_rms_attenuation = new TH2D ("rms_attenuation","Rms_Attenuation; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_rms_time_difference_CFD = new TH2D ("rms_time_difference_CFD","Rms_Time_Difference_CFD; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_rms_time_difference_max = new TH2D ("rms_time_difference_max","Rms_Time_Difference_max; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_expected_length = new TH2D ("expected_length","expected_length; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_time_difference = new TH2D ("expected_timing","Expected_Timing; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_length_difference = new TH2D ("length_difference","Length_Difference; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  TH2D *h2_real_length = new TH2D ("real_length","Real_Length; Y; Z", slot_tot_number+2, -1, slot_tot_number+1, channel_tot_number+2, -1, channel_tot_number+1);
  ...
  for (int c = 0; c < 2; ++c) {
    ...
    stringstream ss_side,ss_wall,ss_col;
    ss_side << s;ss_wall << w;ss_col << c;

    string filename_ = string("root_files_run")+run+string("/Run")+run+string("_s")+ss_side.str()+string("_w")+ss_wall.str()+string("_col")+ss_col.str()+string(".root");
  ...
  for (unsigned i = 0; i<tab_length.size(); i++){//16 channel
    for (unsigned j = 0; j<tab_length[i].size(); j++){//21 slot
      if (j==9||j==10) {
        ...
        h2_attenuation_charge->SetBinContent(j+2,i+2,(charge_second_peak_map[i][j]/charge_first_peak_map[i][j])/.83);
